...
COPY temp_editor(name) FROM 'C:\\editors.csv'
    WITH (
        FORMAT csv,
        DELIMITER ';',
        HEADER true
...
DO $$
    DECLARE
        csv_path VARCHAR(255) := 'C:/projects/';
    BEGIN
        CREATE TEMPORARY TABLE temp_editor(name VARCHAR(255));
        CREATE TEMPORARY TABLE temp_author_role(role_name VARCHAR(50));
        CREATE TEMPORARY TABLE temp_author(author_name VARCHAR(255));
        CREATE TEMPORARY TABLE temp_book(book_id INT, book_name VARCHAR(255), price NUMERIC(9,2), edition_year INT, editor_name VARCHAR(255));
    ...
    EXECUTE 'COPY temp_author(author_name) FROM ' || quote_literal(concat(csv_path, 'author.csv')) || 'WITH (FORMAT csv, HEADER true)';
...
SELECT * FROM catalog.author;

SELECT * FROM catalog.book;

SELECT * FROM catalog.AuthorBook;

SELECT * FROM catalog.AuthorRole;
